...
deltaMethod(
  object = coef_values, 
  vcov. = vcov_matrix, 
  g = "-(b_asc + b_wq_local_basin*(2-1) - (b_wq_local_basin*2))/b_cost"
)

deltaMethod(
  object = coef_values, 
  vcov. = vcov_matrix, 
  g = "-((b_asc + b_wq_nonlocal_basin*(5-1)) - (b_wq_nonlocal_basin*5))/b_cost"
)

deltaMethod(
  object = coef_values, 
  vcov. = vcov_matrix, 
  g = "-((b_asc + b_wq_local_sub_basin*(5-1)) - (b_wq_local_sub_basin*5))/b_cost"
)

deltaMethod(
  object = coef_values, 
  vcov. = vcov_matrix, 
  g = "-((b_asc + b_wq_nonlocal_sub_basin*(5-1)) - (b_wq_nonlocal_sub_basin*5))/b_cost"
)
