...
database <- database %>%
  filter(!is.na(VOTE))%>%
  filter(!is.na(WQ_POLICY_CHOICE))%>%
  filter(!is.na(WQ_CURRENT_CHOICE))
...
apollo_probabilities = function(apollo_beta, apollo_inputs, functionality = "estimate") {
  ...
  V[["policy"]]  = b_asc + 
                   b_cost *COST + 
                   b_wq*WQ_POLICY_CHOICE +  
                   b_asc_version2*VERSION_2 +
                   b_asc_version3*VERSION_3 +
                   b_asc_version4*VERSION_4 +
    
                   b_wq_x_version2*WQ_POLICY_CHOICE*VERSION_2 +
                   b_wq_x_version3*WQ_POLICY_CHOICE*VERSION_3 +
                   b_wq_x_version4*WQ_POLICY_CHOICE*VERSION_4 
    
    
  V[["opt_out"]] = b_wq*WQ_CURRENT_CHOICE +
                   b_wq_x_version2*WQ_CURRENT_CHOICE*VERSION_2 +
                   b_wq_x_version3*WQ_CURRENT_CHOICE*VERSION_3 +
                   b_wq_x_version4*WQ_CURRENT_CHOICE*VERSION_4
