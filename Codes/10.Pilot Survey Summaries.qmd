...
df <- read_csv(here("Deriveddata", "processed_pilotdata_1_Apollo.csv"))
...
df <- read_sav(here("Rawdata", "Water_Quality.sav"))
...
# Plot
ggplot(df2, aes(x = factor(SURVEY_CONTENT, levels = c("Environment", "Movies", "Politics")), 
                y = Percentage, fill = SURVEY_CONTENT)) +
...
age_percentages_movie <- df_movie %>%
  filter(!is.na(age)) %>%
  count(age) %>%
...
# Calculate percentages = Movies
age_percentages_politis <- df_politis %>%
  filter(!is.na(age)) %>%
  count(age) %>%
  mutate(percentage = n / sum(n) * 100)%>%
  mutate(type = "Politics")
...
# Calculate percentages = Movies
age_percentages_environment <- df_environment %>%
  filter(!is.na(age)) %>%
  count(age) %>%
  mutate(percentage = n / sum(n) * 100)%>%
...
age <- rbind(age_percentages_movie,age_percentages_politis,age_percentages_environment)



ggplot(age, aes(x = factor(age), y = percentage, fill = type)) +
...
gender_environment <- df_environment %>%
  filter(!is.na(gender)) %>%
  count(gender) %>%
  mutate(percentage = n / sum(n) * 100)%>%
  mutate(type = "Environment")

geneder <- rbind(gender_movie,gender_politics,gender_environment)




ggplot(geneder, aes(x = factor(gender), y = percentage, fill = type)) +
...
income_percentages_movie <- df_movie %>%
  filter(!is.na(income)) %>%
  count(income) %>%
  mutate(percentage = n / sum(n) * 100)%>%
  mutate(type = "Movies")

income_percentages_politics <- df_politis %>%
  filter(!is.na(income)) %>%
  count(income) %>%
  mutate(percentage = n / sum(n) * 100)%>%
  mutate(type = "Politics")
...
income_percentages_environment <- df_environment %>%
  filter(!is.na(income)) %>%
  count(income) %>%
  mutate(percentage = n / sum(n) * 100)%>%
  mutate(type = "Environment" )
...
income <- rbind(income_percentages_movie,income_percentages_politics,income_percentages_environment)


ggplot(income, aes(x = factor(income), y = percentage, fill = type)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8),width = 0.5) +  # Dodge for side-by-side bars
  labs(x = "", y = "Percentage (%)", title = "") +
  scale_x_discrete(labels = c("1" = "Strongly agree", "2" = "Agree", "3" = "Unsure", "4" = "Disagree", "5" = "Strongly disagree")) +
  scale_fill_manual(values = c("Movies" = "darkblue", "Politics" = "maroon", "Environment" = "darkgreen")) +
...
df8_Movies_A <- df8_Movies%>%
  filter(column == "A1")%>%
  select(value,percentage)%>%
  rename("Movies" = "percentage")


df8_Politics_A <- df8_Politics%>%
  filter(column == "A1")%>%
  select(value,percentage)%>%
  rename("Politics" = "percentage")

df8_Environment_A <- df8_Environment%>%
  filter(column == "A1")%>%
  select(value,percentage)%>%
  rename("Environment" = "percentage")

df_movie <- df8_Movies_A%>%
  left_join(df8_Politics_A)%>%
  left_join(df8_Environment_A)



df_long <- df_movie %>%
  pivot_longer(cols = -value, names_to = "Type", values_to = "Percentage")


# Create side-by-side bar plot
p1 <- ggplot(df_long, aes(x = factor(value), y = Percentage, fill = Type)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8),width = 0.4) +  # Dodge for side-by-side bars
...
# Create side-by-side bar plot
p2 <- ggplot(df_long, aes(x = factor(value), y = Percentage, fill = Type)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8),width = 0.4) +  # Dodge for side-by-side bars
  labs(x = "", y = "Percentage", title = "A2 : Humans are severely abusing the planet") +
  scale_x_discrete(labels = c("1" = "Strongly agree", "2" = "Agree", "3" = "Unsure", "4" = "Disagree", "5" = "Strongly disagree")) +
  scale_fill_manual(values = c("Movies" = "darkblue", "Politics" = "maroon", "Environment" = "darkgreen")) +
  theme_minimal() +  # Apply theme_minimal() once
  theme(
...
# Create side-by-side bar plot
p3 <- ggplot(df_long, aes(x = factor(value), y = Percentage, fill = Type)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8),width = 0.4) +  # Dodge for side-by-side bars
  labs(x = "", y = "Percentage", title = "A3 : Plants and animals have the same right to exist as humans") +
  scale_x_discrete(labels = c("1" = "Strongly agree", "2" = "Agree", "3" = "Unsure", "4" = "Disagree", "5" = "Strongly disagree")) +
  scale_fill_manual(values = c("Movies" = "darkblue", "Politics" = "maroon", "Environment" = "darkgreen")) +
  theme_minimal() +  # Apply theme_minimal() once
  theme(
...
# Create side-by-side bar plot
p4 <- ggplot(df_long, aes(x = factor(value), y = Percentage, fill = Type)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8),width = 0.4) +  # Dodge for side-by-side bars
  labs(x = "", y = "Percentage", title = "A4 : Nature is strong enough to cope with the impacts of modern industrial nations") +
  scale_x_discrete(labels = c("1" = "Strongly agree", "2" = "Agree", "3" = "Unsure", "4" = "Disagree", "5" = "Strongly disagree")) +
  scale_fill_manual(values = c("Movies" = "darkblue", "Politics" = "maroon", "Environment" = "darkgreen")) +
  theme_minimal() +  # Apply theme_minimal() once
  theme(
...
# Create side-by-side bar plot
p5 <- ggplot(df_long, aes(x = factor(value), y = Percentage, fill = Type)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8),width = 0.4) +  # Dodge for side-by-side bars
  labs(x = "", y = "Percentage", title = "A5 : Humans were meant to rule over the rest of nature") +
  scale_x_discrete(labels = c("1" = "Strongly agree", "2" = "Agree", "3" = "Unsure", "4" = "Disagree", "5" = "Strongly disagree")) +
  scale_fill_manual(values = c("Movies" = "darkblue", "Politics" = "maroon", "Environment" = "darkgreen")) +
  theme_minimal() +  # Apply theme_minimal() once
  theme(
...
# Create side-by-side bar plot
p6 <- ggplot(df_long, aes(x = factor(value), y = Percentage, fill = Type)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8),width = 0.4) +  # Dodge for side-by-side bars
  labs(x = "", y = "Percentage", title = "A6 : The balance of nature is very delicate and easily upset") +
  scale_x_discrete(labels = c("1" = "Strongly agree", "2" = "Agree", "3" = "Unsure", "4" = "Disagree", "5" = "Strongly disagree")) +
  scale_fill_manual(values = c("Movies" = "darkblue", "Politics" = "maroon", "Environment" = "darkgreen")) +
  theme_minimal() +  # Apply theme_minimal() once
  theme(
    ...
    axis.line = element_line(),  # Keep axis lines visible
    plot.title = element_text(size = 8),
    legend.key.size = unit(0.2, "cm")  # Change the size of the legend marks
  )
...
ggplot(df9, aes(x = value, y = percentage, fill = value)) +
  geom_bar(stat = "identity", position = "dodge",width = 0.2) +  # "dodge" for side-by-side bars
  labs(
    title = "",
    y = "Percentage",
    fill = "Response Value"  # Rename the legend here
...
10. ***Q20.When you decided to live in your current house, did you consider the water quality in nearby rivers or streams?***
