...
database <- database%>%
  ...
  #filter(!all(VOTE == 1)) %>%
  #mutate(across(c(Q15_GENERAL_THOUGHTS_CH2, Q15_GENERAL_THOUGHTS_CH4, Q15_GENERAL_THOUGHTS_CH6),
  #              ~ replace_na(.x, 0))) %>%
  
  #filter(!if_any(c(Q15_GENERAL_THOUGHTS_CH2, Q15_GENERAL_THOUGHTS_CH4, Q15_GENERAL_THOUGHTS_CH6), ~ .x == 1)) %>%
  #mutate(across(c(Q12_VOTE_WITHOUT_CONSIDER_OTHER),
                #~ replace_na(.x, 0))) %>%
  filter(Q12_VOTE_WITHOUT_CONSIDER_OTHER %in% c (2,3))%>% # Voted on each proposal without any consideration of other proposals 
  filter(Q12_VOTE_CERTAIN_PUBLIC_ELEC %in% c(2,3))%>% # I am certain that I voted the same way I would if I were voting in a public election
  ungroup()
...
df_all <- rbind(df1,df2,df3,df4)%>%
  mutate(Validity_Q = "Q12")


write.csv(df_all, "Tables/WTP_oneUnit_after_removeing_Q12.csv")
